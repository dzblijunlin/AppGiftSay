 ...
 * 攻略页面->品类->查看全部
 */
public class CateStrategyTypeGvInfoActivity extends BaseActivity {
    ...
    @Override
    int setlayout() {
        return R.layout.activity_cate_strategy_type_gv_info;
    }
    ...
    void initData() {
        ...
        NetHelper.MyRequest(url, CateStrategyTypeAllBean.class, new NetListener<CateStrategyTypeAllBean>() {
            ...
            public void successListener(CateStrategyTypeAllBean response) {
                List<CateStrategyTypeAllBean.DataBean.ChannelGroupsBean.ChannelsBean> data = response.getData().getChannel_groups().get(0).getChannels();
                adapter.setData(data);
                rv.setAdapter(adapter);
                GridLayoutManager manager = new GridLayoutManager(CateStrategyTypeGvInfoActivity.this,2);
                rv.setLayoutManager(manager);
                titleTv.setText(response.getData().getChannel_groups().get(0).getName());
            }
